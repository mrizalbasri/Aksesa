...
export function getCreditsBalance(token: string): Promise<CreditsBalance> {
  return requestJson<CreditsBalance>("/api/v1/credits/balance", { token });
}
...
export function getCreditsHistory(
  ...
  return requestJson<CreditsHistory>(
...
export function upgradeSubscription(
  ...
  return requestJson<CreditsBalance>(
    `/api/v1/credits/upgrade?tier=${tier}&credits=${credits}`,
    {
      method: "POST",
      token,
    }
